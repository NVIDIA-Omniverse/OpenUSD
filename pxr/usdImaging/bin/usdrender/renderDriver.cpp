...
bool RenderAll(const Options&o,const StageData&d,const RenderRequest&r){
 ...
 engine.SetEnablePresentation(false);engine.SetRendererSetting(HdRenderSettingsTokens->enableInteractive,VtValue(false));
 engine.SetActiveRenderSettingsPrimPath(r.settings);if(!r.pass.IsEmpty())engine.SetActiveRenderPassPrimPath(r.pass);
 for(const auto&v:r.customSettings)engine.SetRendererSetting(TfToken(v.first),v.second);engine.SetRendererAov(HdAovTokens->color);
 std::vector<UsdTimeCode>frames;if(!ParseFrames(o,d.stage->GetStartTimeCode(),&frames))return false;
 const UsdGeomCamera camera(d.stage->GetPrimAtPath(r.camera));std::cout<<"Camera: "<<r.camera<<"\nRenderer plugin: "<<engine.GetCurrentRendererId()<<"\n";
 const auto purposes=_Split(o.purposes);
 for(const UsdTimeCode time:frames){const GfCamera gc=camera.GetCamera(time);GfVec2i size=r.products[0].resolution;
  if(o.imageWidth>0||size[0]<=0||size[1]<=0){const int w=o.imageWidth>0?o.imageWidth:960;size=GfVec2i(w,std::max(1,(int)(w/std::max(.0001f,gc.GetAspectRatio()))));}
  engine.SetCameraPath(r.camera);const GfRange2f ndc=r.products[0].dataWindow;const GfVec2i lo((int)std::floor(ndc.GetMin()[0]*size[0]),(int)std::floor((1.0f-ndc.GetMax()[1])*size[1]));const GfVec2i hi((int)std::ceil(ndc.GetMax()[0]*size[0])-1,(int)std::ceil((1.0f-ndc.GetMin()[1])*size[1])-1);const GfRange2f display(GfVec2f(0),GfVec2f(size));engine.SetFraming(CameraUtilFraming(display,GfRect2i(lo,hi),r.products[0].pixelAspectRatio));engine.SetRenderBufferSize(size);
  const GfFrustum frustum=gc.GetFrustum();GlfSimpleLightVector lights;if(o.cameraLight){const GfVec3d pos=frustum.GetPosition();GlfSimpleLight l(GfVec4f(pos[0],pos[1],pos[2],1));l.SetTransform(frustum.ComputeViewInverse());lights.push_back(l);}
  GlfSimpleMaterial material;engine.SetLightingState(lights,material,GfVec4f(.01f,.01f,.01f,1));
  std::vector<std::string>expected;{UsdEditContext edit(d.stage,d.session);for(const RenderProduct&product:r.products){std::string path,error;if(!ResolveOutputPath(product.name,o.outputRoot,time,&path,&error)){std::cerr<<error<<"\n";return false;}if(!UsdRenderProduct(d.stage->GetPrimAtPath(product.path)).GetProductNameAttr().Set(TfToken(path))){std::cerr<<"Could not override productName on <"<<product.path<<">\n";return false;}if(_Exists(path)&&ArchUnlinkFile(path.c_str())!=0){std::cerr<<"Could not remove stale RenderProduct '"<<path<<"'\n";return false;}expected.push_back(path);}}
  UsdImagingGLRenderParams rp;rp.frame=time;rp.complexity=o.complexity;rp.colorCorrectionMode=TfToken(o.colorCorrection);rp.clearColor=GfVec4f(0);
