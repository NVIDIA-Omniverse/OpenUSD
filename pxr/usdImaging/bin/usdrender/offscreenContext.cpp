    ...
    : _impl(new Impl), _enabled(enabled)
{
    ...
    GLXFBConfig *configs = glXChooseFBConfig(
        _impl->display, DefaultScreen(_impl->display), attrs, &count);
    if (!configs || count == 0) {
        _error = "Could not choose a GLX framebuffer configuration";
        if (configs)
            XFree(configs);
        return;
    }
    const int pbAttrs[] = {GLX_PBUFFER_WIDTH, 1, GLX_PBUFFER_HEIGHT, 1, None};
    _impl->pbuffer = glXCreatePbuffer(_impl->display, configs[0], pbAttrs);
    _impl->context = glXCreateNewContext(_impl->display, configs[0],
                                         GLX_RGBA_TYPE, nullptr, True);
    XFree(configs);
    if (!_impl->pbuffer || !_impl->context ||
        !glXMakeContextCurrent(_impl->display, _impl->pbuffer, _impl->pbuffer,
                               _impl->context))
...
OffscreenContext::~OffscreenContext()
{
    ...
    if (!_impl)
        return;
    if (_impl->display) {
        glXMakeContextCurrent(_impl->display, None, None, nullptr);
        if (_impl->context)
            glXDestroyContext(_impl->display, _impl->context);
        if (_impl->pbuffer)
            glXDestroyPbuffer(_impl->display, _impl->pbuffer);
