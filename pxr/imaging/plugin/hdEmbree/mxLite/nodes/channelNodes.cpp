...
static void
_EvalCombine2(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
              MxLiteNodeOutputMap* outputs)
{
    float x = MxLiteGet<float>(inputs, _tokens->in1, 0.0f);
    float y = MxLiteGet<float>(inputs, _tokens->in2, 0.0f);
    (*outputs)[_tokens->out] = VtValue(GfVec2f(x, y));
}

static void
_EvalCombine3_color3(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
                     MxLiteNodeOutputMap* outputs)
{
    float x = MxLiteGet<float>(inputs, _tokens->in1, 0.0f);
    float y = MxLiteGet<float>(inputs, _tokens->in2, 0.0f);
    float z = MxLiteGet<float>(inputs, _tokens->in3, 0.0f);
    (*outputs)[_tokens->out] = VtValue(GfVec3f(x, y, z));
}

static void
_EvalCombine4(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
              MxLiteNodeOutputMap* outputs)
{
    float x = MxLiteGet<float>(inputs, _tokens->in1, 0.0f);
    float y = MxLiteGet<float>(inputs, _tokens->in2, 0.0f);
    float z = MxLiteGet<float>(inputs, _tokens->in3, 0.0f);
    float w = MxLiteGet<float>(inputs, _tokens->in4, 0.0f);
...
static void
_EvalSeparate2(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
               MxLiteNodeOutputMap* outputs)
{
    GfVec2f v = MxLiteGet<GfVec2f>(inputs, _tokens->in, GfVec2f(0.0f));
    (*outputs)[_tokens->outx] = VtValue(v[0]);
    (*outputs)[_tokens->outy] = VtValue(v[1]);
}

static void
_EvalSeparate3_color3(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
                      MxLiteNodeOutputMap* outputs)
{
...
static void
_EvalSeparate4(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
               MxLiteNodeOutputMap* outputs)
{
...
static void
_EvalExtract_vec2(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
                  MxLiteNodeOutputMap* outputs)
{
    GfVec2f v = MxLiteGet<GfVec2f>(inputs, _tokens->in, GfVec2f(0.0f));
    int idx   = MxLiteGet<int>(inputs, _tokens->index, 0);
    idx = std::clamp(idx, 0, 1);
    (*outputs)[_tokens->out] = VtValue(v[idx]);
}

static void
_EvalExtract_vec3(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
                  MxLiteNodeOutputMap* outputs)
{
    GfVec3f v = MxLiteGet<GfVec3f>(inputs, _tokens->in, GfVec3f(0.0f));
    int idx   = MxLiteGet<int>(inputs, _tokens->index, 0);
...
static void
_EvalExtract_vec4(const MxLiteParamMap& inputs, const MxLiteShadingContext&,
                  MxLiteNodeOutputMap* outputs)
{
    GfVec4f v = MxLiteGet<GfVec4f>(inputs, _tokens->in, GfVec4f(0.0f));
    int idx   = MxLiteGet<int>(inputs, _tokens->index, 0);
...
static void
_EvalConvert_float_color3(const MxLiteParamMap& inputs,
                          const MxLiteShadingContext&,
                          MxLiteNodeOutputMap* outputs)
{
...
static void
_EvalConvert_color3_float(const MxLiteParamMap& inputs,
                          const MxLiteShadingContext&,
                          MxLiteNodeOutputMap* outputs)
{
    GfVec3f v = MxLiteGet<GfVec3f>(inputs, _tokens->in, GfVec3f(0.0f));
...
static void
_EvalConvert_color3_vector3(const MxLiteParamMap& inputs,
                            const MxLiteShadingContext&,
                            MxLiteNodeOutputMap* outputs)
{
    GfVec3f v = MxLiteGet<GfVec3f>(inputs, _tokens->in, GfVec3f(0.0f));
...
static void
_EvalConvert_color4_vector4(const MxLiteParamMap& inputs,
                            const MxLiteShadingContext&,
                            MxLiteNodeOutputMap* outputs)
{
    GfVec4f v = MxLiteGet<GfVec4f>(inputs, _tokens->in, GfVec4f(0.0f));
...
static void
_EvalConvert_float_color4(const MxLiteParamMap& inputs,
                          const MxLiteShadingContext&,
                          MxLiteNodeOutputMap* outputs)
{
    float v = MxLiteGet<float>(inputs, _tokens->in, 0.0f);
...
static void
_EvalConvert_integer_float(const MxLiteParamMap& inputs,
                           const MxLiteShadingContext&,
                           MxLiteNodeOutputMap* outputs)
{
    int v = MxLiteGet<int>(inputs, _tokens->in, 0);
...
void
MxLiteRegisterChannelNodes(MxLiteNodeRegistry& reg)
{
