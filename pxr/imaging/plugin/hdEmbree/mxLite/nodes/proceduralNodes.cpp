...
static void
_EvalNoise3d_float(const MxLiteParamMap& inputs,
                   const MxLiteShadingContext& ctx,
                   MxLiteNodeOutputMap* outputs)
{
    GfVec3f pos = MxLiteGet<GfVec3f>(inputs, _tokens->position, ctx.position);
    float amp   = MxLiteGet<float>(inputs, _tokens->amplitude, 1.0f);
    float pivot = MxLiteGet<float>(inputs, _tokens->pivot,     0.0f);
    float n = _PerlinNoise3d(pos[0], pos[1], pos[2]);
    (*outputs)[_tokens->out] = VtValue(pivot + n * amp);
}

static void
_EvalNoise3d_color3(const MxLiteParamMap& inputs,
                    const MxLiteShadingContext& ctx,
                    MxLiteNodeOutputMap* outputs)
{
    GfVec3f pos = MxLiteGet<GfVec3f>(inputs, _tokens->position, ctx.position);
    GfVec3f amp = MxLiteGet<GfVec3f>(inputs, _tokens->amplitude, GfVec3f(1.0f));
    GfVec3f pivot = MxLiteGet<GfVec3f>(inputs, _tokens->pivot, GfVec3f(0.0f));
    GfVec3f n = _PerlinNoise3d_vec3(pos[0], pos[1], pos[2]);
    (*outputs)[_tokens->out] = VtValue(pivot + GfCompMult(n, amp));
}

static void
_EvalNoise2d_float(const MxLiteParamMap& inputs,
                   const MxLiteShadingContext& ctx,
                   MxLiteNodeOutputMap* outputs)
{
    GfVec2f tc = MxLiteGet<GfVec2f>(inputs, _tokens->texcoord, ctx.texcoord);
    float amp  = MxLiteGet<float>(inputs, _tokens->amplitude, 1.0f);
    float pivot = MxLiteGet<float>(inputs, _tokens->pivot, 0.0f);
...
static void
_EvalFractal3d_float(const MxLiteParamMap& inputs,
                     const MxLiteShadingContext& ctx,
                     MxLiteNodeOutputMap* outputs)
{
    GfVec3f pos    = MxLiteGet<GfVec3f>(inputs, _tokens->position, ctx.position);
    float amp      = MxLiteGet<float>(inputs, _tokens->amplitude,  1.0f);
    int   octaves  = MxLiteGet<int>(inputs, _tokens->octaves, 3);
    float lacunarity = MxLiteGet<float>(inputs, _tokens->lacunarity, 2.0f);
    float diminish   = MxLiteGet<float>(inputs, _tokens->diminish,   0.5f);
...
static void
_EvalCellnoise3d(const MxLiteParamMap& inputs,
                 const MxLiteShadingContext& ctx,
                 MxLiteNodeOutputMap* outputs)
{
    GfVec3f pos = MxLiteGet<GfVec3f>(inputs, _tokens->position, ctx.position);
...
static void
_EvalCellnoise2d(const MxLiteParamMap& inputs,
                 const MxLiteShadingContext& ctx,
                 MxLiteNodeOutputMap* outputs)
{
    GfVec2f tc = MxLiteGet<GfVec2f>(inputs, _tokens->texcoord, ctx.texcoord);
...
void
MxLiteRegisterProceduralNodes(MxLiteNodeRegistry& reg)
{
