...
#define _REG(name) MxLiteTest_Register("Graph." #name, &name)
...
TestCompileEmptyNetwork()
{
    ...
    auto graph = MxLiteEvalGraph::Compile(network);
...
TestCompileSingleTerminal()
{
    ...
    MxLiteSurfaceClosure closure = graph->Evaluate(ctx);

    if (!MxLiteTest_IsClose(closure.baseColor, GfVec3f(1, 0, 0), 1e-4f)) {
        printf("    baseColor: (%f,%f,%f)\n",
               closure.baseColor[0], closure.baseColor[1], closure.baseColor[2]);
        return false;
    }
    return MxLiteTest_IsClose(closure.roughness, 0.3f);
}
...
TestCompileLinearChain()
{
    ...
    if (!MxLiteTest_IsClose(closure.roughness, 2.0f)) {
...
TestCompileDiamondDAG()
{
    ...
    // 2.0 + 3.0 = 5.0
    if (!MxLiteTest_IsClose(closure.roughness, 5.0f)) {
...
TestEvalWithConstantInputs()
{
    ...
    if (!MxLiteTest_IsClose(closure.baseColor, GfVec3f(0, 1, 0), 1e-4f)) {
        printf("    baseColor: (%f,%f,%f)\n",
               closure.baseColor[0], closure.baseColor[1], closure.baseColor[2]);
        return false;
    }
    if (!MxLiteTest_IsClose(closure.roughness, 0.4f)) return false;
    if (!MxLiteTest_IsClose(closure.metallic, 0.5f)) return false;
    return true;
}
...
TestEvalMultiplyChain()
{
    ...
    if (!MxLiteTest_IsClose(closure.baseColor, GfVec3f(6, 6, 6), 1e-4f)) {
        printf("    baseColor: (%f,%f,%f)\n",
               closure.baseColor[0], closure.baseColor[1], closure.baseColor[2]);
        return false;
    }
...
TestEvalGeometricInput()
{
    ...
    if (!MxLiteTest_IsClose(closure.baseColor, GfVec3f(0.3f, 0.6f, 0.9f), 1e-4f)) {
        printf("    baseColor: (%f,%f,%f)\n",
               closure.baseColor[0], closure.baseColor[1], closure.baseColor[2]);
        return false;
    }
...
TestInvalidGraphEvaluate()
{
    ...
    return MxLiteTest_IsClose(closure.baseColor, GfVec3f(0.8f), 1e-4f);
}
