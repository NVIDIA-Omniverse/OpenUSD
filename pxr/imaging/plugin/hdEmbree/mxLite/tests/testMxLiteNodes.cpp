...
#define _REG(name) MxLiteTest_Register("Nodes." #name, &name)

// Helper: evaluate a registered node with given inputs.
static MxLiteNodeOutputMap
_Eval(const char* nodeTypeId, const MxLiteParamMap& inputs)
{
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken(nodeTypeId));
...
static float _GetFloat(const MxLiteNodeOutputMap& o, const char* name = "out") {
...
static GfVec3f _GetVec3(const MxLiteNodeOutputMap& o, const char* name = "out") {
    auto it = o.find(TfToken(name));
...
static GfVec2f _GetVec2(const MxLiteNodeOutputMap& o, const char* name = "out") {
    auto it = o.find(TfToken(name));
...
static bool TestAddFloat() {
    ...
    return MxLiteTest_IsClose(_GetFloat(out), 5.0f);
}
...
static bool TestAddColor3() {
    ...
    return MxLiteTest_IsClose(_GetVec3(out), GfVec3f(5, 7, 9));
}
...
static bool TestMultiplyFloat() {
    ...
    return MxLiteTest_IsClose(_GetFloat(out), 12.0f);
}
...
static bool TestSubtractFloat() {
    ...
    return MxLiteTest_IsClose(_GetFloat(out), 7.0f);
}
...
static bool TestDivideFloat() {
    ...
    if (!MxLiteTest_IsClose(_GetFloat(out), 2.5f)) return false;

    // Division by zero should be safe.
    in[TfToken("in2")] = VtValue(0.0f);
    out = _Eval("ND_divide_float", in);
    return MxLiteTest_IsClose(_GetFloat(out), 0.0f);
}
...
static bool TestClamp() {
    ...
    if (!MxLiteTest_IsClose(_GetFloat(out), 1.0f)) return false;
...
static bool TestMix() {
    ...
    if (!MxLiteTest_IsClose(_GetFloat(out), 0.0f)) return false;

    in[TfToken("mix")] = VtValue(1.0f);
    out = _Eval("ND_mix_float", in);
    if (!MxLiteTest_IsClose(_GetFloat(out), 10.0f)) return false;
...
static bool TestSmoothstep() {
    ...
    in[TfToken("in")] = VtValue(0.5f);
    out = _Eval("ND_smoothstep_float", in);
    return MxLiteTest_IsClose(_GetFloat(out), 0.5f);
}
...
static bool TestRemap() {
    ...
    return MxLiteTest_IsClose(_GetFloat(out), 15.0f);
}
...
static bool TestCombineSeparateRoundtrip() {
    ...
    return MxLiteTest_IsClose(x, 0.2f) &&
           MxLiteTest_IsClose(y, 0.4f) &&
           MxLiteTest_IsClose(z, 0.6f);
}
...
static bool TestIfgreater() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken("ND_ifgreater_float"));
    if (!fn) {
    ...
    MxLiteShadingContext ctx;
    fn(in, ctx, &out);
    return MxLiteTest_IsClose(_GetFloat(out), 10.0f);
}
...
static bool TestGeometricPosition() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken("ND_position_vector3"));
    if (!fn) return false;
    ...
    MxLiteNodeOutputMap out;
    fn(in, ctx, &out);
    return MxLiteTest_IsClose(_GetVec3(out), GfVec3f(1.0f, 2.0f, 3.0f));
}

static bool TestGeometricNormal() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken("ND_normal_vector3"));
    if (!fn) return false;
    ...
    MxLiteNodeOutputMap out;
    fn(in, ctx, &out);
    return MxLiteTest_IsClose(_GetVec3(out), GfVec3f(0.0f, 1.0f, 0.0f));
}
...
static bool TestLuminance() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken("ND_luminance_color3"));
    if (!fn) {
    ...
    if (!MxLiteTest_IsClose(result, 1.0f, 0.01f)) {
    ...
    if (!MxLiteTest_IsClose(result, expected, 0.001f)) {
...
static bool TestNodeRegistryLookup() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(TfToken("ND_add_float"));
    return fn != nullptr;
}

static bool TestNodeRegistryMissing() {
    MxLiteNodeRegistry::RegisterBuiltinNodes();
    auto fn = MxLiteNodeRegistry::GetInstance().Find(
