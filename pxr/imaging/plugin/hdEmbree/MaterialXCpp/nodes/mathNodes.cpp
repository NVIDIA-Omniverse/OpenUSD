...
_EvalMultiply(const ParamMap& inputs, const ShadingContext&,
              NodeOutputMap* outputs)
{
    T a = Get<T>(inputs, _tokens->in1, One<T>());
    T b = Get<T>(inputs, _tokens->in2, One<T>());
    (*outputs)[_tokens->out] = VtValue(CompMul(a, b));
}
...
{
    T a = Get<T>(inputs, _tokens->in1, One<T>());
    float b = Get<float>(inputs, _tokens->in2, 1.0f);
    (*outputs)[_tokens->out] = VtValue(a * b);
}
            ...
            NodeOutputMap* outputs)
{
    T a = Get<T>(inputs, _tokens->in1, Zero<T>());
    T b = Get<T>(inputs, _tokens->in2, One<T>());
    (*outputs)[_tokens->out] = VtValue(CompDiv(a, b));
}
              ...
              NodeOutputMap* outputs)
{
    T a = Get<T>(inputs, _tokens->in1, Zero<T>());
    float b = Get<float>(inputs, _tokens->in2, 1.0f);
    (*outputs)[_tokens->out] = VtValue(b != 0.0f ? a / b : Zero<T>());
}
...
_EvalModulo(const ParamMap& inputs, const ShadingContext&,
            NodeOutputMap* outputs)
{
    float a = Get<float>(inputs, _tokens->in1, 0.0f);
    float b = Get<float>(inputs, _tokens->in2, 1.0f);
...
_EvalPower(const ParamMap& inputs, const ShadingContext&,
           NodeOutputMap* outputs)
{
    float base = Get<float>(inputs, _tokens->in1, 0.0f);
    float exp  = Get<float>(inputs, _tokens->in2, 1.0f);
...
_EvalLn(const ParamMap& inputs, const ShadingContext&,
        NodeOutputMap* outputs)
{
    float v = Get<float>(inputs, _tokens->in, 1.0f);
...
static void _EvalSin(const ParamMap& in, const ShadingContext&,
                     NodeOutputMap* out) {
    (*out)[_tokens->out] = VtValue(std::sin(Get<float>(in, _tokens->in, 0.0f)));
}
static void _EvalCos(const ParamMap& in, const ShadingContext&,
                     NodeOutputMap* out) {
    (*out)[_tokens->out] = VtValue(std::cos(Get<float>(in, _tokens->in, 0.0f)));
}
static void _EvalTan(const ParamMap& in, const ShadingContext&,
                     NodeOutputMap* out) {
    (*out)[_tokens->out] = VtValue(std::tan(Get<float>(in, _tokens->in, 0.0f)));
}
static void _EvalAsin(const ParamMap& in, const ShadingContext&,
                      NodeOutputMap* out) {
    float v = std::clamp(Get<float>(in, _tokens->in, 0.0f), -1.0f, 1.0f);
...
static void _EvalAtan2(const ParamMap& in, const ShadingContext&,
                       NodeOutputMap* out) {
    float y = Get<float>(in, _tokens->in1, 0.0f);
    float x = Get<float>(in, _tokens->in2, 1.0f);
...
_EvalDotProduct(const ParamMap& inputs, const ShadingContext&,
                NodeOutputMap* outputs)
{
    GfVec3f a = Get<GfVec3f>(inputs, _tokens->in1, GfVec3f(0.0f));
    GfVec3f b = Get<GfVec3f>(inputs, _tokens->in2, GfVec3f(0.0f));
...
_EvalNormalize(const ParamMap& inputs, const ShadingContext&,
               NodeOutputMap* outputs)
{
    GfVec3f v = Get<GfVec3f>(inputs, _tokens->in, GfVec3f(0.0f, 0.0f, 1.0f));
...
_EvalConstant(const ParamMap& inputs, const ShadingContext&,
              NodeOutputMap* outputs)
{
    T v = Get<T>(inputs, _tokens->value, Zero<T>());
    (*outputs)[_tokens->out] = VtValue(v);
}
