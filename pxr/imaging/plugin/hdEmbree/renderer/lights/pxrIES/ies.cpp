...
void IESFile::pack(float *data)
{
  if (v_angles.size() && h_angles.size()) {
    ...
    *(data++) = sizet_to_float(v_angles.size());
    ...
    for (size_t h = 0; h < intensity.size(); h++) {
...
bool IESFile::parse(const string &ies)
{
  ...
  // The UsdLux specification is photometric so for those purposes, skip the
  // cd/W conversion.
...
void IESFile::process_type_b()
{
  ...
  for (size_t i = 0; i < v_angles.size(); i++) {
    ...
    for (size_t j = 0; j < h_angles.size(); j++) {
...
void IESFile::process_type_a()
{
  ...
  for (size_t i = h_angles.size() - 1; i >= 0; i--) {
  ...
  if (angle_close(h_angles[0], 0.0f)) {
    ...
    for (size_t i = 1; i < h_angles.size(); i++) {
      new_h_angles.push_back(180.0f + h_angles[i]);
      new_intensity.push_back(intensity[i]);
    }
  }
