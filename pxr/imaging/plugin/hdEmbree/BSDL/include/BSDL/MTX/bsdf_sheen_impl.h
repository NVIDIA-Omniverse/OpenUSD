...
ContyKullaDist<SHD_LEGACY>::D(const Imath::V3f& Hr) const
{
    ...
    return BSDL_CONFIG::Fast::powf(sin_theta, 1 / a) * (2 + 1 / a) * 0.5f
           * ONEOVERPI;
}
...
ContyKullaDist<SHD_LEGACY>::get_lambda(float cosNO) const
{
    ...
    const float pivot = a / (1 + b * BSDL_CONFIG::Fast::powf(0.5f, c)) + d * 0.5f
                        + e;

    float p = a / (1 + b * BSDL_CONFIG::Fast::powf(x, c)) + d * x + e;
    if (cosNO > 0.5f)  // Mirror around 0.5f
        p = 2 * pivot - p;
    // That curve fits lambda in log scale, now exponentiate
    return BSDL_CONFIG::Fast::expf(p);
}
...
ContyKullaDist<SHD_LEGACY>::G2(const Imath::V3f& wo, const Imath::V3f& wi) const
{
    ...
    if constexpr (SHD_LEGACY) {
        ...
        lambdaI = BSDL_CONFIG::Fast::powf(lambdaI,
...
ZeltnerBurleySheen::fetch_coeffs(float cosNO) const
{
    if (FITTED_LTC) {
        ...
        float R = BSDL_CONFIG::Fast::expf(-0.5f * q * q) * invs * inv_sqrt2pi
                  + o;
        assert(isfinite(A));
        assert(isfinite(B));
        assert(isfinite(R));
